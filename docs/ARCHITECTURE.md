...
| `health_logger` (daemon) | `core.py:512` | reads via `republish_state` -> `snapshot_state` (`:462-463`, `parsers.py:612`) | `AVAILABILITY_ONLINE` (`:436`) |

`STATE_LOCK` is an `RLock` (`state.py:21`) held only inside `snapshot_state` and `update_state` (`:75-85`), never across I/O. The comment at `state.py:76` records why: a dict resize during `on_connect` iteration killed the paho thread silently. `health_logger` ticks every 10 s (`core.py:512`), runs the watchdog each tick (`:454`), republishes retained state when `heartbeat_due` (`parsers.py:597`, interval `max(UPDATE_INTERVAL_SEC, EXPIRE_AFTER_SEC//3)` = 600 s at defaults), and prints `[HEALTH]` every third tick (`:467-469`). Availability is keyed on decoded telemetry (`state.py:32`, `core.py:411`), not on packet age, with a timeout of `max(TELEMETRY_TIMEOUT_SEC, min(3 x observed gap, 3600))` (`core.py:372-399`) and a startup grace measured from `PROCESS_START_TS` (`:412-415`).
...
**What only the smoke test proves**: the `__main__` body (`core.py:580-622`), `start_mqtt` (`mqtt.py:320-325`), and that the image starts at all. **What nothing proves**: the ARP send path (`core.py:172-207`, `:50-54`, always mocked at `tests/test_core.py:63-68`, disabled in smoke at `scripts/smoke-test.sh:39`); `SIGTERM -> shutdown` (`core.py:572-577` is only asserted callable at `tests/test_core.py:373-374`, and smoke tears down with `docker rm -f`, `scripts/smoke-test.sh:25`); the `health_logger` loop body (`core.py:512`); `run.sh` itself (entrypoint overridden at `scripts/smoke-test.sh:46`; only grepped by `tests/test_packaging.py:97-100`). One test is vacuous: `tests/test_core.py:309-314` patches `core.client` but `shutdown` publishes through `mqtt.py`'s own client (`core.py:591` -> `mqtt.py:179-181`), so the `except` at `core.py:536` never runs.
